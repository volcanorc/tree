...
- A pannable and zoomable family map with circular portraits and SVG relationship connectors.
...
- An independent pet archive with optional human owners, pet partners, and offspring.
...
5. Add portrait images under `public/portraits/` when using repository paths.
...
- `people`: stable IDs, public details, portrait locations, links, and protection state.
...
Relative portrait paths resolve from the `/tree/` project base. External portraits must use HTTPS, and external story links must use HTTP or HTTPS.
...
Protected records can be edited but not deleted. Non-protected leaves can be deleted. A parent with descendants cannot be removed if doing so would leave those descendants without a parent.
